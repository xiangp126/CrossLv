...
# server local ip
local_ip=(
# mark local ip of server here, one per line
"127.0.0.1"
)
...
for ((i = 0; i < ${#server_ip[@]}; ++i)); do
    ...
    # ./jungle.sh [ssh_ip] [ssh_port] [ssh_user] [ssh_home] [ssh_prompt] [ssh_passwd] [nic_port_name]
    ./fstack.exp ${server_ip[$i]} $ssh_port $ssh_user $ssh_home $ssh_prompt $ssh_passwd ${nic_port_name[$i]}
    # Parallel executing, with &/wait
    # ./jungle.exp ${server_ip[$i]} $ssh_port $ssh_user $ssh_home $ssh_prompt $ssh_passwd ${nic_port_name[$i]} &
done
